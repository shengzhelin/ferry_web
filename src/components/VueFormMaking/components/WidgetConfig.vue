<template>
  <div v-if="show">
    <el-form label-position="top">
      ...
      >
        <el-checkbox v-model="data.options.labelWidthDisabled">自定義</el-checkbox>
      ...
      <template v-if="data.type==='text'">
        <el-form-item label="文字內容">
          <el-input v-model="data.options.defaultValue" placeholder="請輸入文字內容" />
        </el-form-item>
        <el-form-item label="文字大小">
          <el-input v-model="data.options.font_size" placeholder="請輸入字體大小" />
        </el-form-item>
        <el-form-item label="文字顏色">
          <el-input v-model="data.options.font_color" placeholder="請輸入文字顏色" />
        </el-form-item>
        <el-form-item label="設置粗體">
          <el-input v-model="data.options.font_weight" placeholder="請輸入設置粗體" />
        </el-form-item>
        <el-form-item label="字體屬性">
          <el-input v-model="data.options.font_family" placeholder="請輸入字體屬性" />
        </el-form-item>
      </template>
      <!-- 分割符 -->
      <template v-if="data.type==='divider'">
        ...
        <el-form-item label="文字位置">
          <el-radio-group v-model="data.options.content_position">
            ...
            <el-radio-button label="center">中間</el-radio-button>
            <el-radio-button label="right">右</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="分割線方向">
          <el-radio-group v-model="data.options.direction">
            <el-radio-button label="horizontal">橫</el-radio-button>
            <el-radio-button label="vertical">豎</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="文字大小">
          <el-input v-model="data.options.font_size" placeholder="請輸入字體大小" />
      ...
      <el-form-item v-if="Object.keys(data.options).indexOf('displayVerifiy')>=0" :label="$t('fm.config.widget.displayVerifiy')">
        <el-radio-group v-model="data.options.displayVerifiy.type">
          <el-radio label="hide">不校驗</el-radio>
          <el-radio label="and">與</el-radio>
        ...
        <div v-if="data.options.displayVerifiy.type !== 'hide'">
          <template v-for="(item, index) in data.options.displayVerifiy.list">
            <div :key="item.model">
              ...
              <el-button v-if="index > 0" type="text" icon="el-icon-remove-outline" @click="delDisplayVerifiy(index)">刪  除</el-button>
    ...
    >
      <div>
        <el-form ref="addTreeData" :model="addTreeData" label-width="80px">
          <el-form-item
            prop="label"
            label="Label"
            :rules="{ required: true, message: 'Label不能為空', trigger: 'blur' }"
          >
          ...
          <el-form-item
            prop="value"
            label="Value"
            :rules="{ required: true, message: 'Value不能為空', trigger: 'blur' }"
          >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="operatingStatus==='add'?appendCascaderData():cascaderDialog = false">確 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
