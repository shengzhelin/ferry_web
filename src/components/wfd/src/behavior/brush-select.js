...
module.exports = {
  ...
  getEvents() {
    ...
    if (ALLOW_EVENTS.indexOf(this.trigger.toLowerCase()) > -1) {
      ...
      trigger = DEFAULT_TRIGGER
      console.warn('Behavior brush-select的trigger參數不合法，請輸入drag、shift、ctrl或alt')
    }
    if (trigger === 'drag') {
      return {
  ...
  _getSelectedNodes(e) {
    ...
    if (this.includeEdges) {
      // 選中邊，邊的source和target都在選中的節點中時才選中
  ...
  onKeyDown(e) {
    const code = e.key
    // 按住control鍵時，允許用戶設置trigger為ctrl
  ...
  onKeyUp() {
    if (this.brush) {
      // 清除所有選中狀態後，設置拖得動狀態為false，並清除框選的brush
      this.brush.destroy()
      this.brush = null
      this.dragging = false
    }
