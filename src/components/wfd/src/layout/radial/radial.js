...
Layout.registerLayout('radial', {
  ...
  handleAbnormalMatrix(adMatrix, focusIndex) {
    ...
    // 空行即代表該行是離散點，將單個離散點看作 focus 的鄰居
    for (let i = 0; i < rows; i++) {
  ...
  handleInfinity(matrix, focusIndex, step) {
    ...
    // 遍歷 matrix 中遍歷 focus 對應行
    for (let i = 0; i < length; i++) {
      // matrix 關注點對應行的 Inf 項
      if (matrix[focusIndex][i] === Infinity) {
        matrix[focusIndex][i] = step
        matrix[i][focusIndex] = step
        // 遍歷 matrix 中的 i 行，i 行中非 Inf 項若在 focus 行為 Inf，則替換 focus 行的那個 Inf
    ...
    // 處理其他行的 Inf。根據該行對應點與 focus 距離以及 Inf 項點 與 focus 距離，決定替換值
    for (let i = 0; i < length; i++) {
      if (i === focusIndex) {
        continue
      }
